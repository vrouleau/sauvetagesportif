...
function decodeHeatStatus(s: number | null): 'empty' | 'assigned' | 'completed' {
...
const CORE_TABLES = [
  'swimstyle', 'club', 'swimsession', 'athlete',
...
const PG_SCHEMA_DDL: string[] = [
  `CREATE TABLE IF NOT EXISTS swimstyle (
    swimstyleid INTEGER NOT NULL, code VARCHAR(10), distance SMALLINT,
    name VARCHAR(50), relaycount SMALLINT, stroke SMALLINT, sortcode INTEGER,
    technique SMALLINT, uniqueid SMALLINT,
    CONSTRAINT pk_swimstyle PRIMARY KEY (swimstyleid))`,
  `CREATE TABLE IF NOT EXISTS club (
    clubid INTEGER NOT NULL, bonuspoints INTEGER, clubtype SMALLINT,
    code VARCHAR(10), name VARCHAR(80), nation VARCHAR(3),
    CONSTRAINT pk_club PRIMARY KEY (clubid))`,
  `CREATE TABLE IF NOT EXISTS swimsession (
    swimsessionid INTEGER NOT NULL, course SMALLINT,
    daytime TIMESTAMP WITHOUT TIME ZONE, endtime TIMESTAMP WITHOUT TIME ZONE,
    feeathlete DOUBLE PRECISION, following CHAR(1) DEFAULT 'F',
    lanemin SMALLINT, lanemax SMALLINT, name VARCHAR(100),
    officialmeeting TIMESTAMP WITHOUT TIME ZONE, roundtotenths CHAR(1) DEFAULT 'F',
    sessionnumber SMALLINT, timing SMALLINT, touchpadmode SMALLINT,
    warmupfrom TIMESTAMP WITHOUT TIME ZONE, warmupuntil TIMESTAMP WITHOUT TIME ZONE,
    remarks TEXT, remarksjury TEXT, maxentriesathlete SMALLINT,
    maxentriesrelay SMALLINT, poolglobal CHAR(1) DEFAULT 'F',
    CONSTRAINT pk_swimsession PRIMARY KEY (swimsessionid))`,
  `CREATE TABLE IF NOT EXISTS athlete (
    athleteid INTEGER NOT NULL, clubid INTEGER, firstname VARCHAR(30),
    gender SMALLINT, lastname VARCHAR(50), birthdate TIMESTAMP WITHOUT TIME ZONE,
    domicile VARCHAR(50), license VARCHAR(20), nation VARCHAR(3),
    CONSTRAINT pk_athlete PRIMARY KEY (athleteid),
    CONSTRAINT fk_athlete_club FOREIGN KEY (clubid) REFERENCES club(clubid))`,
  `CREATE TABLE IF NOT EXISTS swimevent (
    swimeventid INTEGER NOT NULL, eventnumber SMALLINT, gender SMALLINT,
    round SMALLINT, sortcode INTEGER, swimsessionid INTEGER, swimstyleid INTEGER,
    internalevent CHAR(1) DEFAULT 'F', masters CHAR(1) DEFAULT 'F',
    roundname VARCHAR(50), splashmecanedit CHAR(1) DEFAULT 'F',
    pfineignore CHAR(1) DEFAULT 'F', seedbonuslast CHAR(1) DEFAULT 'F',
    seedexhlast CHAR(1) DEFAULT 'F', seedlateentrylast CHAR(1) DEFAULT 'F',
    seedingglobal CHAR(1) DEFAULT 'F', twoperlane CHAR(1) DEFAULT 'F',
    combineagegroups CHAR(1) DEFAULT 'F', daytime TIMESTAMP WITHOUT TIME ZONE,
    CONSTRAINT pk_swimevent PRIMARY KEY (swimeventid),
    CONSTRAINT fk_swimevent_swimsession FOREIGN KEY (swimsessionid) REFERENCES swimsession(swimsessionid) ON DELETE CASCADE,
    CONSTRAINT fk_swimevent_swimstyle FOREIGN KEY (swimstyleid) REFERENCES swimstyle(swimstyleid))`,
  `CREATE TABLE IF NOT EXISTS agegroup (
    agegroupid INTEGER NOT NULL, swimeventid INTEGER, name VARCHAR(50),
    agemin SMALLINT, agemax SMALLINT, gender SMALLINT, heatcount SMALLINT,
    sortcode INTEGER, useformedals CHAR(1) DEFAULT 'F',
    useforscoring CHAR(1) DEFAULT 'F', allofficial CHAR(1) DEFAULT 'F',
    agebytotal CHAR(1) DEFAULT 'F', forceprelim CHAR(1) DEFAULT 'F',
    seedwithtsonly CHAR(1) DEFAULT 'F',
    CONSTRAINT pk_agegroup PRIMARY KEY (agegroupid),
    CONSTRAINT fk_agegroup_swimevent FOREIGN KEY (swimeventid) REFERENCES swimevent(swimeventid) ON DELETE CASCADE)`,
  `CREATE TABLE IF NOT EXISTS heat (
    heatid INTEGER NOT NULL, heatnumber SMALLINT, racestatus SMALLINT,
    sortcode INTEGER, swimeventid INTEGER, name VARCHAR(50),
    CONSTRAINT pk_heat PRIMARY KEY (heatid),
    CONSTRAINT fk_heat_swimevent FOREIGN KEY (swimeventid) REFERENCES swimevent(swimeventid) ON DELETE CASCADE)`,
  `CREATE TABLE IF NOT EXISTS swimresult (
    swimresultid INTEGER NOT NULL, athleteid INTEGER, agegroupid INTEGER,
    entrytime INTEGER, heatid INTEGER, lane SMALLINT, reactiontime SMALLINT,
    resultstatus SMALLINT, swimeventid INTEGER, swimtime INTEGER,
    usetimetype SMALLINT DEFAULT 0, qttime INTEGER,
    CONSTRAINT pk_swimresult PRIMARY KEY (swimresultid),
    CONSTRAINT fk_swimresult_athlete FOREIGN KEY (athleteid) REFERENCES athlete(athleteid),
    CONSTRAINT fk_swimresult_swimevent FOREIGN KEY (swimeventid) REFERENCES swimevent(swimeventid) ON DELETE CASCADE)`,
  `CREATE TABLE IF NOT EXISTS split (
    swimresultid INTEGER NOT NULL, distance SMALLINT NOT NULL, swimtime INTEGER,
    CONSTRAINT pk_split PRIMARY KEY (swimresultid, distance),
    CONSTRAINT fk_split_swimresult FOREIGN KEY (swimresultid) REFERENCES swimresult(swimresultid) ON DELETE CASCADE)`,
]
...
export async function syncUp(): Promise<{ tablesCreated: string[] }> {
  ...
  const tables = [
    { name: 'swimstyle', pk: 'swimstyleid', cols: ['swimstyleid','code','distance','name','relaycount','stroke','sortcode'] },
    { name: 'club', pk: 'clubid', cols: ['clubid','code','name','nation'] },
    { name: 'swimsession', pk: 'swimsessionid', cols: ['swimsessionid','sessionnumber','name','course','daytime','endtime','lanemin','lanemax','timing','touchpadmode','roundtotenths','warmupfrom','warmupuntil','officialmeeting','remarks','remarksjury','maxentriesathlete','maxentriesrelay','feeathlete','poolglobal','following'] },
    { name: 'athlete', pk: 'athleteid', cols: ['athleteid','clubid','firstname','lastname','birthdate','gender','nation','license','domicile'] },
    { name: 'swimevent', pk: 'swimeventid', cols: ['swimeventid','swimsessionid','eventnumber','gender','round','swimstyleid','sortcode','internalevent','masters','roundname','splashmecanedit','pfineignore','seedbonuslast','seedexhlast','seedlateentrylast','seedingglobal','twoperlane','combineagegroups','daytime'] },
    { name: 'agegroup', pk: 'agegroupid', cols: ['agegroupid','swimeventid','name','agemin','agemax','gender','heatcount','sortcode','useformedals','useforscoring','allofficial','agebytotal','forceprelim','seedwithtsonly'] },
    { name: 'heat', pk: 'heatid', cols: ['heatid','swimeventid','heatnumber','racestatus','sortcode','name'] },
    { name: 'swimresult', pk: 'swimresultid', cols: ['swimresultid','athleteid','agegroupid','entrytime','heatid','lane','reactiontime','resultstatus','swimeventid','swimtime','usetimetype','qttime'] },
...
export async function syncDown(): Promise<{ rowsCopied: number }> {
  ...
  const tables = [
    { name: 'swimstyle', cols: ['swimstyleid','code','distance','name','relaycount','stroke','sortcode'] },
    { name: 'club', cols: ['clubid','code','name','nation'] },
    { name: 'swimsession', cols: ['swimsessionid','sessionnumber','name','course','daytime','endtime','lanemin','lanemax','timing','touchpadmode','roundtotenths','warmupfrom','warmupuntil','officialmeeting','remarks','remarksjury','maxentriesathlete','maxentriesrelay','feeathlete','poolglobal','following'] },
    { name: 'athlete', cols: ['athleteid','clubid','firstname','lastname','birthdate','gender','nation','license','domicile'] },
    { name: 'swimevent', cols: ['swimeventid','swimsessionid','eventnumber','gender','round','swimstyleid','sortcode','internalevent','masters','roundname','splashmecanedit','pfineignore','seedbonuslast','seedexhlast','seedlateentrylast','seedingglobal','twoperlane','combineagegroups','daytime'] },
    { name: 'agegroup', cols: ['agegroupid','swimeventid','name','agemin','agemax','gender','heatcount','sortcode','useformedals','useforscoring','allofficial','agebytotal','forceprelim','seedwithtsonly'] },
    { name: 'heat', cols: ['heatid','swimeventid','heatnumber','racestatus','sortcode','name'] },
    { name: 'swimresult', cols: ['swimresultid','athleteid','agegroupid','entrytime','heatid','lane','reactiontime','resultstatus','swimeventid','swimtime','usetimetype','qttime'] },

  ...
  return (window as unknown as {
    api?: {
      db?: {
        ...
        updateSession: (sessionId: number, data: Record<string, unknown>) => Promise<{ ok: boolean }>
        deleteSession: (id: number) => Promise<{ ok: boolean }>
        createEvent: (
          sessionId: number, number: number, gender: string,
          distance: number, phase: string, styleName: string,
        ) => Promise<{ id: number }>
        createBreak: (sessionId: number, number: number, name: string) => Promise<{ id: number }>
        deleteEvent: (id: number) => Promise<{ ok: boolean }>
        updateEvent: (eventId: number, data: Record<string, unknown>) => Promise<{ ok: boolean }>
        createAgeGroup: (
          eventId: number, name: string, minAge: number, maxAge: number | null, gender: string,
        ) => Promise<{ id: number }>
        deleteAgeGroup: (id: number) => Promise<{ ok: boolean }>
        updateAgeGroup: (agegroupId: number, data: Record<string, unknown>) => Promise<{ ok: boolean }>
        getMeetConfig: () => Promise<Record<string, string>>
        setMeetConfig: (entries: Record<string, { type: string; value: string }>) => Promise<{ ok: boolean }>
        getSwimStyles: () => Promise<Array<{ id: number; distance: number; name: string }>>
        reorderEvents: (updates: Array<{ eventId: number; sessionId: number; sortcode: number }>) => Promise<{ ok: boolean }>
      }
    }
  }).api?.db ?? null
}
...
export default function EventsPage({ refreshKey = 0 }: { refreshKey?: number }) {
  ...
  const [meetName, setMeetName] = useState<string>(competition.nameFr)
  ...
  async function handleDragEnd(event: DragEndEvent) {
    ...
    if (sourceSession.id === targetSession.id) {
      ...
      dbApi()?.reorderEvents([...srcUpdates, ...tgtUpdates])
    }
  }
  ...
  async function handleDelete() {
    ...
    try {
      if (selected.type === 'session') {
        if (api) await api.deleteSession(selected.session.id)
        ...
        if (api) await api.deleteEvent(event.id)
        setLocalSessions((prev) =>
          prev.map((s) =>
        ...
        if (api) await api.deleteAgeGroup(group.id)
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <div className="w-[480px] shrink-0 border-r border-gray-300 overflow-y-auto bg-white select-none text-xs">
          ...
          >
            ...
            <span className="w-28" />
            <span className="w-14" />
            <span className="w-14 text-center">{t.events.poolUnit(competition.poolSize)}</span>
          </div>
...
function AgeGroupPropertiesPanel({ group, event }: { group: AgeGroup; event: CompetitionEvent }) {
  ...
  function save(data: Record<string, unknown>) {
    const api = dbApi()
    if (api) api.updateAgeGroup(group.id, data)
  }
...
function EventPropertiesPanel({ event }: { event: CompetitionEvent }) {
  ...
  const [swimStyles, setSwimStyles] = useState<Array<{ id: number; distance: number; name: string }>>([])
  ...
  function save(data: Record<string, unknown>) {
    const api = dbApi()
    if (api) api.updateEvent(event.id, data)
  }
...
function CompetitionPropertiesPanel({ onMeetNameChange }: { onMeetNameChange: (name: string) => void }) {
  ...
  useEffect(() => {
    const api = dbApi()
    if (!api?.getMeetConfig) { setLoading(false); return }
    api.getMeetConfig().then((cfg) => {
      setMeetValues(cfg ?? {})
      setLoading(false)
    }).catch(() => setLoading(false))
